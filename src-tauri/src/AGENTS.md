...
| `profile.rs` | `Profile` (serde mirror of the TS type) and `build_invocation()`. On Windows it builds a `wsl.exe [-d distro] [--cd cwd] -- bash …` command; on Unix it runs `bash` directly. Login shell `-l`, or `-lic <script>` with optional `exec bash -l` keep-open. Unit-tested for both platforms. |
