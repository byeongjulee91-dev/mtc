...
| File | Description |
|------|-------------|
| `main.rs` | App entry. Declares the modules, defines the eight `#[tauri::command]`s (`load_app_data`, `save_app_data`, `discover_skills`, `count_git_modified`, `create_session`, `write_session`, `resize_session`, `close_session`), registers the dialog plugin, manages a `PtyManager`, and runs the Tauri builder. Hides the console on Windows release builds. |
| `git.rs` | `count_modified_files()` — counts *modified* (changed) git-tracked files (`git status --porcelain -uno`, untracked excluded) for a project dir: `Ok(None)` empty path, `Ok(Some(n))` a repo, `Err(reason)` on failure (git's first stderr line, e.g. not-a-repo / dubious-ownership) so the UI can surface it. Linux-style paths on Windows are counted inside WSL (reusing `storage`'s `is_linux_path` / `to_wsl_root_expr`); native paths run `git` directly (both `CREATE_NO_WINDOW`). Unit-tested. |
