...
| `git.rs` | `count_tracked_files()` — counts git-tracked files (`git ls-files -z`) for a project dir, returning `None` for a non-repo / missing git so the UI hides the badge. Linux-style paths on Windows are counted inside WSL (reusing `storage`'s `is_linux_path` / `to_wsl_root_expr`); native paths run `git` directly. Unit-tested. |
